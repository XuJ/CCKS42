...
def run_post_processing(data_dir, result_file, test_dir):
  output_file = '{}_single.txt'.format(result_file.split('.')[0])
  output_file2 = '{}_full.txt'.format(result_file.split('.')[0])
  test_file = os.path.join(test_dir, 'event_element_dev_data.txt')
  ...
  role_entity_type_dict = {
    ...
    '质押结束日期': '时间', '增持金额': '数字&单位', '增持的股东': '公司/人名', '增持开始日期': '时间', '减持金额': '数字&单位', '减持的股东': '公司/人名',
    '减持开始日期': '时间',
  }
...
def main():
  ...
  parser.add_argument("--dir", required=True, help="pred data dir")
  parser.add_argument("--result", required=True, help="pred file name")
  parser.add_argument("--test", required=True, help="test raw data dir")
  args = parser.parse_args()
  run_post_processing(args.dir, args.result, args.test)
